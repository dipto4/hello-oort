...
def initialize():
    ...
    #new conditions for the nice model follow below

    semi_major = [5,7.5,15,12.5] 

    for i in range(1,5):
        theta  = i*0.25*math.pi

        solar_system[i].x = (semi_major[i-1]*math.cos(theta)) | units.AU 
        solar_system[i].y = (semi_major[i-1]*math.sin(theta)) | units.AU 
        solar_system[i].z = 0 | units.AU 

        velocity_squared = (constants.G * solar_system[0].mass / (semi_major[i-1] | units.AU))
        velocity = math.sqrt(velocity_squared.value_in(units.km **2 / units.s **2))
        vx = -velocity*math.sin(theta)
        vy = velocity*math.cos(theta)

        solar_system[i].vx = (vx) | (units.km / units.s) 
        solar_system[i].vy = vy | (units.km / units.s)
        solar_system[i].vz = 0 | (units.km / units.s)
    ...
    for i in range(0,number_of_particles):
       radius = (random.random()*(disk_end-disk_start) + disk_start) | units.AU
...
def planetplot():
    ...
    write_set_to_file(solar_system,'evolutionSolarSystem_w3r1_nice_0.hdf5','hdf5')
    write_set_to_file(test_particles,'evolutionParticles_w3r1_nice_0.hdf5','hdf5')
    
    t_end= 10 | units.Myr
    time=0 | units.yr
    count = 0
    while time<t_end:
        time=time+ (500 | units.yr)
        err=mer.evolve_model(time)
        for particle in mer.particles:
            if (particle.x == (0 | units.AU) and particle.y == (0 | units.AU) and particle.z == (0 |units.AU) and particle.vx == (0 |units.km / units.s) and particle.vy == (0 |units.km / units.s) and particle.vz == (0 | units.km / units.s)):
                particle.name='ignore'
        channel.copy()
